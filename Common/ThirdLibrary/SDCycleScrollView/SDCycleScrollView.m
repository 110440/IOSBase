...
- (NSArray*)getImagesAndViews{
    NSMutableArray * ret = @[].mutableCopy;
    [self.imageURLStringsGroup enumerateObjectsUsingBlock:^(NSString* obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSIndexPath * path = [NSIndexPath indexPathForRow:idx inSection:0];
        SDCollectionViewCell * cell = (SDCollectionViewCell*)[self.mainView cellForItemAtIndexPath:path];
        NSDictionary * dic = @{
...
- (void)makeScrollViewScrollToIndex:(NSInteger)index{
    ...
    if (0 == _totalItemsCount) return;
    int targetIndex = (int)(_totalItemsCount * 0.5 + index);
    if (!_infiniteLoop) {
